        ...
        <div class="flex flex-col lg:flex-row">
            ...
            <div class="lg:w-2/5 p-5 flex flex-col relative">
                <div class="aspect-w-16 aspect-h-12 rounded-2xl overflow-hidden bg-slate-100 relative shadow-inner">
                    ...
                    @if($stay->is_luxury)
                    <div class="absolute bottom-4 left-4 bg-amber-400 text-slate-900 text-[10px] px-4 py-1.5 rounded-full font-black uppercase tracking-widest flex items-center shadow-2xl animate-pulse">
            ...
            <div class="lg:w-3/5 p-6 lg:pl-2 flex flex-col">
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                    <div>
                        <h2 class="text-3xl font-black text-slate-900 mb-1 tracking-tight leading-tight uppercase">{{ $stay->name }}</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="bg-slate-50/50 rounded-2xl border border-slate-100 overflow-hidden">
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-sm font-black text-slate-800">₹{{ number_format($stay->single_sharing_rent) }}</span>
                            </div>
                            @endif
                            ...
                            <div class="flex justify-between items-center py-2 px-3 bg-white rounded-xl border border-blue-100 shadow-sm ring-2 ring-blue-50">
                                <div class="flex flex-col">
                                    <span class="text-[9px] font-black text-blue-600 uppercase tracking-wider">Most Popular</span>
                                    <span class="text-xs font-black text-slate-800">Double Sharing</span>
                                </div>
                                <span class="text-lg font-black text-blue-700">₹{{ number_format($stay->double_sharing_rent) }}</span>
                            </div>

                            @if($stay->triple_sharing_rent)
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-slate-500">Triple Sharing</span>
                                <span class="text-sm font-black text-slate-800">₹{{ number_format($stay->triple_sharing_rent) }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50/50 rounded-2xl border border-slate-100 overflow-hidden">
                        <div class="bg-slate-100/50 px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                            <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Meal Services</span>
                        ...
                        <div class="p-4 h-full flex flex-col justify-center">
                            @if($stay->food_type !== 'None')
                                <div class="flex flex-col items-center text-center">
                                    <span class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">{{ $stay->food_type }}</span>
                                    @php
                                    ...
                                    @endphp
                                    <div class="text-xl font-black text-slate-800 mb-1">
                                        @if($min == $max)
                ...
                <!-- Footer Action -->
                <div class="mt-auto">
                    <button onclick="bookVisit({{ json_encode($stay) }})" class="w-full bg-slate-900 hover:bg-blue-700 text-white font-black py-4 px-8 rounded-2xl transition-all duration-300 shadow-xl shadow-slate-200 flex items-center justify-center group active:scale-95 uppercase tracking-widest text-[10px]">
...
@empty
    <div class="text-center py-20 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-100">
        ...
        <h3 class="text-2xl font-black text-slate-800 mb-3 tracking-tight">NO STAYS MATCH YOUR FILTERS</h3>
        ...
        <button onclick="window.location.reload()" class="mt-8 text-blue-600 font-black text-xs uppercase tracking-widest hover:text-blue-800 transition-colors">
